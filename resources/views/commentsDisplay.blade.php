    ...
    @foreach ($comments->take(5)->reverse() as $comment)
        <div class="comment" data-comment-id="{{ $comment->id }}"
            ...
            <div class="comment-header">
                <img class="avatar user-avatar" style="border-radius: 20%" width="40px" height="40px" src="{{ $comment->avatar }}" alt="{{ $comment->user ? $comment->user->name : 'Anonymous' }}">
                <h6 class="comment-author">{{ $comment->user ? $comment->user->name : 'Anonymous' }}</h6>
            </div>
            <div class="comment-body">
                <p>{{ $comment->body }}</p>
                <a href="#" class="reply-link" style="color: blue !important; margin-left:10px" >Reply</a>
...
<script>
    $(document).ready(function() {
        ...
        $('.reply-link').click(function(e) {
            e.preventDefault();
            // Find the closest .comment-body and add the class to hide it
